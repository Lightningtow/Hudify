...
//    private Identifier texture;// = new Identifier("textures/item/albumart");
    private final Identifier texture = Identifier.of("textures/item/barrier.png");// = new Identifier("textures/item/albumart");
...
//    public CustomhudIconExtender(NativeImageBackedTexture image, Flags flags) {
    public CustomhudIconExtender(Identifier idtexture, Flags flags) {
        super(flags, 0);
...
//        NativeImage img = g_native_image;
        ...
        try {
            Optional<Resource> resource = client.getResourceManager().getResource(TEXTURE_NOT_FOUND);
            if (resource.isPresent())
                img = NativeImage.read(resource.get().getInputStream());
        }
        catch (IOException e) { LogThis(Level.ERROR, String.valueOf(e)); }
//        NativeImage img = null;
...
//        matrices.scale(scale, scale, 0);
        ...
        context.drawTexture(pipeline, texture, 0, 0, width, height, 0, 0, textureWidth, textureHeight, textureWidth, textureHeight);
